...
use self::internal::{Buffer, ToVec};
use std;
...
static PNG_FILE_HEADER: [u8; 8] = [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a];
...
fn init_decoder<R: Read + Seek>(reader: &mut R, req_bpc: usize) -> Result<PngDecoder<R>, Error> {
    ...
    if (hdr.bit_depth != 8 && hdr.bit_depth != 16) || (req_bpc != 8 && req_bpc != 16) {
        return Err(::png::Error::Unsupported(
            "only 8-bit and 16-bit images supported",
        ));
    }
    ...
    loop {
        match &dc.chunk_lentype[4..8] {
            ...
            _ => {
                if chunk_names
                    .iter()
                    .any(|name| &name[..] == &dc.chunk_lentype[4..8])
                {
                    let name = [
                        dc.chunk_lentype[4],
                        dc.chunk_lentype[5],
...
fn read_idat_stream<R: Read>(
    ...
    let mut depaletted = if dc.src_indexed {
        vec![0u8; (dc.w * 3) as usize]
    } else {
        Vec::new()
    };
    let mut cline8 = if dc.tgt_bpc == 8 {
        vec![0u8; srclen as usize]
    } else {
        Vec::new()
    };
    let mut cline16 = if dc.tgt_bpc == 16 {
        vec![0u16; srclen as usize]
    } else {
        Vec::new()
    };
    let mut result8 = if dc.tgt_bpc == 8 {
        vec![0u8; (dc.h * tgtlen) as usize]
    } else {
        Vec::new()
    };
    let mut result16 = if dc.tgt_bpc == 16 {
        vec![0u16; (dc.h * tgtlen) as usize]
    } else {
        Vec::new()
    };
    ...
    match dc.ilace {
        PngInterlace::None => {
            ...
            for _j in 0..dc.h {
                ...
                {
                    let bytes: &[u8] = if dc.src_indexed {
                    ...
                    if dc.tgt_bpc == 8 {
                        ...
                        convert8(
                            &cline8,
                            &mut result8[ti as usize..(ti + tgtlen) as usize],
                            c0,
                            c1,
                            c2,
                            c3,
                        ...
                        convert16(
                            &cline16,
                            &mut result16[ti as usize..(ti + tgtlen) as usize],
                            c0,
                            c1,
                            c2,
                            c3,
        ...
        PngInterlace::Adam7 => {
            ...
            let mut redline8 = if dc.tgt_bpc == 8 {
                vec![0u8; tgtlen as usize]
            } else {
                Vec::new()
            };
            let mut redline16 = if dc.tgt_bpc == 16 {
                vec![0u16; tgtlen as usize]
            } else {
                Vec::new()
            };
                ...
                for j in 0..redh[pass] {
...

                    {
                        ...
                        if dc.tgt_bpc == 8 {
                            bpc8_from_bytes(&bytes, dc.src_bpc, &mut cline8);
                            convert8(
                                &cline8[..(redw[pass as usize] * src_chans) as usize],
                            ...
                            for i in 0..redw[pass] {
                                ...
                                result8[tgt as usize..(tgt + tgt_chans) as usize].copy_from_slice(
                                    &redline8[redi as usize..(redi + tgt_chans) as usize],
                                );
                                redi += tgt_chans;
                            }
                        } else {
                            bpc16_from_bytes(&bytes, dc.src_bpc, &mut cline16);
                            convert16(
                                &cline16[..(redw[pass] * src_chans) as usize],
                            ...
                            for i in 0..redw[pass] {
                                let tgt = tgt_px(i, j, dc.w) * tgt_chans;
                                result16[tgt as usize..(tgt + tgt_chans) as usize].copy_from_slice(
                                    &redline16[redi as usize..(redi + tgt_chans) as usize],
                                );
                                redi += tgt_chans;
                            }
                        }
...
fn a7_red1_to_dst(redx: u32, redy: u32, dstw: u32) -> u32 {
    redy * 8 * dstw + redx * 8
}
fn a7_red2_to_dst(redx: u32, redy: u32, dstw: u32) -> u32 {
    redy * 8 * dstw + redx * 8 + 4
}
fn a7_red3_to_dst(redx: u32, redy: u32, dstw: u32) -> u32 {
    (redy * 8 + 4) * dstw + redx * 4
}
fn a7_red4_to_dst(redx: u32, redy: u32, dstw: u32) -> u32 {
    redy * 4 * dstw + redx * 4 + 2
}
fn a7_red5_to_dst(redx: u32, redy: u32, dstw: u32) -> u32 {
    (redy * 4 + 2) * dstw + redx * 2
}
fn a7_red6_to_dst(redx: u32, redy: u32, dstw: u32) -> u32 {
    redy * 2 * dstw + redx * 2 + 1
}
fn a7_red7_to_dst(redx: u32, redy: u32, dstw: u32) -> u32 {
    (redy * 2 + 1) * dstw + redx
}

// will leave len to the length of next chunk after last idat chunk
fn read_idat_chunks<R: Read>(dc: &mut PngDecoder<R>, len: &mut usize) -> Result<(Vec<u8>), Error> {
...
fn recon(cline: &mut [u8], pline: &[u8], ftype: u8, fstep: u32) -> Result<(), Error> {
    match PngFilter::from_u8(ftype) {
        ...
        Some(PngFilter::Average) => {
            for (c, &p) in (&mut cline[..fstep as usize])
                .iter_mut()
            ...
            unsafe {
                for k in fstep..cline.len() as u32 {
                    *cline.get_unchecked_mut(k as usize) = (*cline.get_unchecked(k as usize))
                        .wrapping_add(
                            ((*cline.get_unchecked((k - fstep) as usize) as u32
                                + *pline.get_unchecked(k as usize) as u32)
                                / 2) as u8,
                        );
                }
            }
        ...
        Some(PngFilter::Paeth) => {
            ...
            unsafe {
                for k in fstep..(cline.len() as u32) {
                    *cline.get_unchecked_mut(k as usize) = (*cline.get_unchecked(k as usize))
                        .wrapping_add(paeth(
                            *cline.get_unchecked((k - fstep) as usize),
...
pub fn write_chunks<W: Write>(
    ...
    if w < 1 || h < 1 || 4/*SRC=RGBA*/ * w * h != data.len() as u32 {
        return Err(::png::Error::InvalidArg(
            "invalid dimensions or data length",
        ));
    }
...
fn write_header<W: Write>(ec: &mut PngEncoder<W>) -> Result<(), Error> {
    ...
    try!(ec.stream.write_all(b"\0\0\0\x0dIHDR"));
    crc.put(b"IHDR");
    try!(ec.stream.write_all(width));
    crc.put(width);
    try!(ec.stream.write_all(height));
    crc.put(height);
    let tmp = [
        8, // bit depth
        match ec.alpha {
...
fn write_custom_chunk<W: Write>(ec: &mut PngEncoder<W>, chunk: &ExtChunk) -> Result<(), Error> {
...
fn write_image_data<W: Write>(ec: &mut PngEncoder<W>) -> Result<(), Error> {
    let (convert, c0, c1, c2, c3) = try!(converter(
        ColFmt::RGBA,
    ...
    while si < ec.h * pixel_size {
        convert(
            &ec.data[si as usize..(si + pixel_size) as usize],
            &mut cline[1..tgt_linesz as usize],
            c0,
            c1,
            c2,
            c3,
        ...
        for i in 1..fstep + 1 {
            filtered_image[(ti + i) as usize] =
                cline[i as usize].wrapping_sub(paeth(0, pline[i as usize], 0));
        }
        for i in fstep + 1..(cline.len() as u32) {
            filtered_image[(ti + i) as usize] = cline[i as usize].wrapping_sub(paeth(
                cline[(i - fstep) as usize],
                pline[i as usize],
                pline[(i - fstep) as usize],
            ));
        }
